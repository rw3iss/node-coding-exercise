...
jsmpeg.prototype.nextFrame = function() {
	...
	while(true) {
		...
		// TODO: look into this next week - @suny @jon - this.buffer sometimes equals: WebGLBuffer {} - EMPTY
		if( !this.buffer || Object.keys(this.buffer).length == 0 ) { return; }
...
jsmpeg.prototype.YCbCrToRGBA = function() {
	...
	for( var row = 0; row < rows; row++ ) {
		for( var col = 0; col < cols; col++ ) {
			...
			// Line 1
			var y1 = 1.1 * pY[yIndex1++];
			var y2 = 1.1 * pY[yIndex1++];
			...
			// Line 2
			var y3 = 1.1 * pY[yIndex2++];
			var y4 = 1.1 * pY[yIndex2++];
...
var
	...
	SHADER_FRAGMENT_YCBCRTORGBA = [
		...
		'void main() {',
			'float y = 1.1 * texture2D(YTexture, texCoord).r;',
